...
def last_posts_list_view(request: HttpRequest) -> HttpResponse:
    ...
    """
    last_posts = Post.objects.filter(published_at__isnull=False)[:3]
...
def posts_search_view(request: HttpRequest) -> HttpResponse:
    ...
    """
    title_search = request.GET.get('title_text')
    searched_posts = Post.objects.filter(title__icontains=title_search)
